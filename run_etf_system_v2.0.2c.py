...
# 热点赛道得分（纯动量版）
# ============================================================
def calculate_momentum_score(etf_df: pd.DataFrame, benchmark_df: pd.DataFrame, symbol: str = None, market_bottom: bool = False) -> float:
    """
    动量得分计算
    ...
    # 防御性ETF排除：在市场底部，抗跌（跌幅<5%）的防御性ETF不应被选中
    # 因为它们在成长股牛市中资金流出，表现差
    ...
    for i, date in enumerate(test_dates):
        ...
        if should_rebalance:
            ...
            for symbol in ETF_POOL:
                ...
                if len(hist) < 120:
                    continue
                score = calculate_momentum_score(hist, benchmark_df, symbol, market_bottom)
                if score > -900:
                    scores[symbol] = score
